...
def generate_local_svg_stats(total_repos, total_stars, lang_counts, active_model="Gemini 3.8 Flash"):
...
def generate_ai_summary(repo_data, api_key):
    # Active Gemini 3.x production & preview endpoints
    models = [
...
"""
    ...
    # Keep URL assembly safe from markdown linkification
    ...
    for model_id, model_label in models:
        ...
        # Try up to 2 times to handle transient 503 high-demand spikes
        for attempt in range(2):
            try:
                ...
                if res.status_code == 200:
                    ...
                    print(f"Model {model_id} hit 503 (high demand). Retrying in 2 seconds...")
                    time.sleep(2)
                    continue
                else:
                    print(f"Model {model_id} attempt {attempt+1} failed with status {res.status_code}: {res.text}")
                    break
            except Exception as e:
                print(f"Exception while calling {model_id} on attempt {attempt+1}: {e}")
                if attempt == 0:
                    time.sleep(2)
                    continue
                break

    return "AI Summary temporarily unavailable.", "Gemini 3 Flash"
...
if __name__ == "__main__":
    ...
    print("Execution complete. Native SVGs and dynamic README updated.")    svg_theme_css = """
    <style>
      .card-bg { fill: #ffffff; stroke: #d0d7de; }
      .card-title { fill: #0969da; }
      .card-text { fill: #1f2328; }
